...
All of the communication between your robot and [LetsRobot](https://letsrobot.tv/)'s servers is done over [HTTP](https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol). We use [socket.io](https://en.wikipedia.org/wiki/Socket.IO) for the data communication, and an [MPEG](https://en.wikipedia.org/wiki/MPEG) stream for the audio and video.
...
http://{control-host}:{control-host-port}
...
http://{relay-host}:{video-host-port}/{streamkey}/{xres}/{yres}/
...
http://letsrobot.tv/get_audio_port/{camera-id}
...
http://{relay-host}:{audio-host-port}/{streamkey}/{xres}/{yres}/
...
ffmpeg -f v4l2 -video_size {xres}x{yres} -i /dev/video{video-device-number} -f mpegts -framerate 25 -codec:v mpeg1video -b:v {kbps}k -bf 0 -muxdelay 0.001 http://{relay-host}:{video-port}/{streamkey}/{xres}/{yres}/
...
ffmpeg -f alsa -ar 44100 -ac {audio-device-number} -i hw:{audio-device-number} -f mpegts -codec:a mp2 -b:a 32k -muxdelay 0.001 http://{relay-host}:{audio-port}/{streamkey}/{xres}/{yres}/
...
To tell the server that our bot is ready to be marked as LIVE, we need to send a video status signal.
